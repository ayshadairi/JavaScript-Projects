...
function getTopping(runningTotal,textl) {
    var toppingTotal = 0;
    var selectedTopping = [];
    var toppingArray = document.getBlementsByclassName("toppings");
    for (var j = O; j < toppingArray.length; j++) {
        if (toppingArray[j].checked) {
            selectedTopping.push(toppingArray[j].value);
            console.log("selected toppind item: ("+toppingArray[j].value+")");
